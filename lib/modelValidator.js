...
function Validator(swagger) {
    ...
    if(swagger) {
        ...
        swagger.validateParams = function(spec, data, allowBlankTarget) {
            var dummyModel = modelify(spec);
            return validate(data, dummyModel, this.allModels, allowBlankTarget, self.customValidators);
        };
    }
}
...
function modelify(spec) {
    // Create a model from the spec, to validate the parameters against
    var dummyModel = {};
    dummyModel.required = [];
    dummyModel.id = 'Params';
    dummyModel.properties = {};
    spec.parameters.forEach(function(paramSpec) {
        dummyModel.properties[paramSpec.name] = paramSpec;
        if (paramSpec.required) {
            dummyModel.required.push(paramSpec.name);
        }
    });
    return dummyModel;
}
    ...
    if(value !== undefined) {
        ...
        if(field.validatedType === 'string') {
            if (field.minLength > 0 || field.maxLength > 0) {
                err = validateMinMaxLength(key, value, field.minLength, field.maxLength);
                if (err) errors.push(err);
            }
        }
        if(field.type === 'integer' || field.type === 'number' || field.validatedType === 'date' || field.validatedType === 'date-time') {
            if (field.minimum || field.maximum) {
                err = validateMinMaxValue(key, value, field.minimum, field.maximum);
                if (err) errors.push(err);
            }
            ...
            if (field.exclusiveMinimum || field.exclusiveMaximum) {
                ...
                err = validateMinMaxValue(key, value, minimum, maximum, true);
                if (err) errors.push(err);
            }
        }

        if (field.enum) {
            err = validateEnums(key, value, field.enum);
            if (err) errors.push(err);
        }
    }
